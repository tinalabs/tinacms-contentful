...
import { ContentfulClientOptions } from "../../apis/contentful";

export class ContentfulApiService {
  constructor(public options: ContentfulClientOptions) {}

  public static DELIVERY_CLIENT_ERROR = "`accessTokens.delivery` or `deliveryClient` was not provided in options";
  public static PREVIEW_CLIENT_ERROR = "`accessTokens.preview` or `previewClient` was not provided in options";
  public static MANAGEMENT_CLIENT_ERROR = "`accessTokens.management` or `managementClient` was undefined";
  ...
  get managementClient(): ClientAPI {
    ...
    else if (!this.m_ManagementClient && this.options.accessTokens.management) {
  ...
  public createManagementClientWithUserAccessToken(userAccessToken: string): ClientAPI {
    this.m_ManagementClient = createManagementClient({
